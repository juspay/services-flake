  ...
  options.services.postgres = lib.mkOption {
    ...
    type = lib.types.submodule ({ config, ... }: {
      options = {
        ...
        pgHbaConf =
          let
            mkStringOption = lib.mkOption { type = lib.types.str; };
            pgHbaConfSubmodules = lib.types.submodule {
              options = {
                type = mkStringOption;
                database = mkStringOption;
                user = mkStringOption;
                address = mkStringOption;
                method = mkStringOption;
              };
            };
          in
          lib.mkOption {
            type = lib.types.listOf pgHbaConfSubmodules;
            default = [ ];
            description = ''
      ...
      in
      {
        ...
        "${cfg.name}-init".command =
          let
            ...
            pgHbaConf = defaultPgHbaConf ++ cfg.pgHbaConf;

            # Convert the pgHbaConf array to a string
            pgHbaColumns = "# TYPE\tDATABASE\tUSER\tADDRESS\tMETHOD\n";
            pgHbaConfString = ''
              # Generated by Nix
              ${pgHbaColumns}
            ...
            setupScript = pkgs.writeShellScriptBin "setup-postgres" ''
              ...
              echo "${pgHbaConfString}" | ${pkgs.util-linux}/bin/column -t -s $'\t' > "$PGDATA/pg_hba.conf"
            '';
          in
          ''
