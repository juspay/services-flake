    ...
    readiness_probe = {
      exec.command =
        ...
        lib.concatStringsSep " && " (
          ...
          ++ lib.optional config.filer.enable (check "http://${config.host}:${toString config.filer.port}/")
          ++ lib.optional config.s3.enable (check "http://${config.host}:${toString config.s3.port}/")
        );
