...
in
{
  options = {
    package = lib.mkOption {
      ...
      apply =
        ...
        if config.extensions != null then
          if builtins.hasAttr "withPackages" postgresPkg then
            postgresPkg.withPackages config.extensions
          else
            builtins.throw ''
    ...
    connectionURI = lib.mkOption {
      ...
      readOnly = true;
      default =
        { dbName, ... }: "postgres://${config.listen_addresses}:${builtins.toString config.port}/${dbName}";
      description = ''
      ...
      let
        ...
        hbaConfString = ''
          ...
          ${lib.concatMapStrings (
            cnf: "  ${cnf.type}\t${cnf.database}\t${cnf.user}\t${cnf.address}\t${cnf.method}\n"
          ) hbaConf}
        '';
      in
      lib.mkOption {
